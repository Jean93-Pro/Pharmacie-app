...
function Ventes({ meds, sales, clients, retours, pharmacieId, pharmacieEmail, notify }) {
  ...
  async function finalizeSale() {
    if (cart.length === 0 || paiementInsuffisant) return;
    const saleMeta = {
      date: todayISO(),
            ...
            {cart.length > 0 && (
              <div className="pay-block">
                <div className="pay-modes">
                  <button type="button" className={`pay-mode-btn ${modePaiement === "especes" ? "pay-mode-active" : ""}`} onClick={() => setModePaiement("especes")}>
                    <Banknote size={14} /> Espèces
                  </button>
                  <button type="button" className={`pay-mode-btn ${modePaiement === "mobile" ? "pay-mode-active" : ""}`} onClick={() => setModePaiement("mobile")}>
  ...
  return (
    <div className="modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-panel receipt-modal">
        ...
        <label className="receipt-thermal-toggle no-print">
          <input type="checkbox" checked={thermique} onChange={(e) => setThermique(e.target.checked)} />
          <Printer size={13} /> Format ticket 80mm (imprimante de caisse)
        </label>
        <div className={`modal-body receipt-print ${thermique ? "receipt-thermal" : ""}`}>
...
// équipe) — qui a fait quoi, et quand. Alimenté automatiquement par
// logAudit() côté firebase.js, sans action requise ici.
const AUDIT_LABELS = {
  medicament_ajoute: { label: "Médicament ajouté", tone: "ok" },
  medicament_modifie: { label: "Médicament modifié", tone: "neutral" },
  medicament_supprime: { label: "Médicament supprimé", tone: "danger" },
  employe_invite: { label: "Employé invité", tone: "ok" },
  employe_retire: { label: "Employé retiré", tone: "danger" },
...
function formatAuditDetails(entry) {
  ...
  switch (entry.action) {
    ...
    case "medicament_supprime":
      return d.nom || d.medId || "";
    case "medicament_modifie":
      return d.champs ? `Champs modifiés : ${d.champs.join(", ")}` : "";
    case "employe_invite":
      return `${d.email || ""} (${d.role || ""})`;
    case "employe_retire":
      return d.uid || "";
    case "retour_enregistre":
      return `${fmtFCFA(d.total || 0)} · ${d.articles || 0} article(s)`;
...
function Journal({ audit }) {
  const [filtre, setFiltre] = useState("Tous");

  const employes = useMemo(
    () => Array.from(new Set(audit.map((a) => a.employeEmail).filter(Boolean))),
    [audit]
  );
  const filtered = useMemo(
    () => (filtre === "Tous" ? audit : audit.filter((a) => a.employeEmail === filtre)),
    [audit, filtre]
  );

  function handleExport() {
    const rows = filtered.map((a) => ({
      "Date": a.createdAt?.toDate ? a.createdAt.toDate().toLocaleString("fr-FR") : "",
      "Action": AUDIT_LABELS[a.action]?.label || a.action,
      "Détails": formatAuditDetails(a),
      "Employé": a.employeEmail || "—",
    }));
    exportToExcel([{ name: "Journal", rows }], `journal-${todayISO()}.xlsx`);
  }
  ...
  return (
    <div className="page">
      <PageHead
        title="Journal"
        sub={`${filtered.length} action(s) enregistrée(s)`}
        action={
          <div className="page-actions">
            <select className="select" value={filtre} onChange={(e) => setFiltre(e.target.value)}>
              <option>Tous</option>
      ...
      {filtered.length === 0 ? (
        <div className="panel"><EmptyRow text="Aucune action enregistrée pour le moment." /></div>
      ) : (
        <div className="table-wrap">
          <table className="table">
            <thead>
              <tr><th>Date</th><th>Action</th><th>Détails</th><th>Employé</th></tr>
            </thead>
            <tbody>
              {filtered.map((a) => {
                const meta = AUDIT_LABELS[a.action] || { label: a.action, tone: "neutral" };
                ...
                return (
                  <tr key={a.id}>
                    ...
                    <td className="td-sub">{formatAuditDetails(a)}</td>
                    <td className="td-sub">{a.employeEmail || "—"}</td>
                  </tr>
                );
              })}
            </tbody>
...
function Style() {
  return (
    <style>{`
      ...
      .nav-active { background: rgba(255,255,255,0.13); color: #ffffff; font-weight: 600; border-left-color: var(--gold); }
      .nav-pill {
      ...
      .page-head h1 { font-family: var(--font-display); font-size: 25px; font-weight: 600; margin: 0; color: var(--teal-deep); letter-spacing: -0.2px; }
      ...
      .stat-icon {
        width: 36px; height: 36px; border-radius: 9px; background: var(--sage);
      ...
      .stat-value { font-size: 20px; font-weight: 600; font-family: var(--font-display); letter-spacing: -0.2px; }
      .stat-label { font-size: 12px; color: var(--ink-soft); margin-top: 1px; }
      .stat-sub { font-size: 11px; color: #8b988f; margin-top: 3px; font-family: var(--font-mono); }

      .two-col { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
      .panel { background: var(--panel); border: 1px solid var(--line); border-radius: 12px; padding: 16px 18px; box-shadow: var(--shadow-sm); }
      .panel-alert { border-color: var(--rose); background: var(--rose-soft); box-shadow: none; }
      .panel-alert .panel-head h3 { color: var(--rose); }
      .panel-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; }
      .panel-head h3 { display: flex; align-items: center; gap: 7px; font-size: 13.5px; margin: 0; color: var(--teal-deep); font-weight: 600; }
      .link-btn { display: flex; align-items: center; gap: 2px; border: none; background: none; color: var(--teal); font-size: 12px; cursor: pointer; font-weight: 600; transition: gap 0.15s ease; }
      .link-btn:hover { gap: 5px; }
      ...
      .badge { display: inline-flex; align-items: center; gap: 4px; padding: 3px 10px; border-radius: 20px; font-size: 10.5px; font-weight: 700; letter-spacing: 0.2px; }
      .badge-ok { background: var(--ok-soft); color: var(--ok); }
      .badge-warn { background: var(--amber-soft); color: var(--amber); }
      .badge-danger { background: var(--rose-soft); color: var(--rose); }
      .badge-neutral { background: #ece9de; color: var(--ink-soft); }

      .icon-btn { width: 28px; height: 28px; border-radius: 6px; border: 1px solid var(--line); background: var(--panel); color: var(--ink-soft); display: flex; align-items: center; justify-content: center; cursor: pointer; transition: background 0.15s ease, color 0.15s ease, border-color 0.15s ease; }
      .icon-btn:hover { background: var(--sage); color: var(--teal-deep); border-color: var(--sage); }
      .icon-danger:hover { background: var(--rose-soft); color: var(--rose); border-color: var(--rose-soft); }

      .btn-primary { display: flex; align-items: center; gap: 6px; background: var(--teal); color: white; border: none; padding: 9px 15px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; transition: background 0.15s ease, box-shadow 0.15s ease; box-shadow: var(--shadow-sm); }
      .btn-primary:hover { background: var(--teal-deep); box-shadow: var(--shadow-md); }
      .btn-primary:disabled { opacity: 0.45; cursor: not-allowed; box-shadow: none; }
      .btn-ghost { display: flex; align-items: center; gap: 6px; background: none; border: 1px solid var(--line); color: var(--ink); padding: 9px 15px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; transition: background 0.15s ease, border-color 0.15s ease; }
      ...
      .modal-backdrop {
        position: fixed; inset: 0; background: rgba(14,20,17,0.5); backdrop-filter: blur(2px);
        display: flex; align-items: center; justify-content: center; z-index: 50;
        animation: fadeIn 0.15s ease;
      }
      .modal-panel {
        background: var(--panel); border-radius: 14px; width: 420px; max-width: 92vw;
      ...
      .modal-head h3 { margin: 0; font-size: 16px; font-weight: 600; font-family: var(--font-display); color: var(--teal-deep); }
      ...
      .pay-mode-btn {
        ...
        padding: 7px 6px; border-radius: 7px; font-size: 11.5px; font-weight: 600; cursor: pointer;
        transition: background 0.15s ease, border-color 0.15s ease, color 0.15s ease;
      }
      ...
      .client-card { background: var(--panel); border: 1px solid var(--line); border-radius: 12px; padding: 14px; box-shadow: var(--shadow-sm); transition: box-shadow 0.18s ease, transform 0.18s ease; }
      ...
      .toast {
        ...
        background: var(--teal-deep); color: white; padding: 10px 16px; border-radius: 9px;
      ...
      .plan-name { font-family: var(--font-display); font-size: 16px; font-weight: 700; color: var(--teal-deep); }
      .plan-price { font-size: 24px; font-weight: 700; font-family: var(--font-display); color: var(--teal); }
      ...
      .receipt-payment { font-size: 11px; color: var(--ink-soft); margin-top: 8px; padding-top: 8px; border-top: 1px dashed var(--line); display: flex; flex-direction: column; gap: 2px; }
      .receipt-footer { text-align: center; font-size: 11px; color: var(--ink-soft); margin-top: 14px; }

      .receipt-thermal-toggle {
        display: flex; align-items: center; gap: 6px; font-size: 11.5px; color: var(--ink-soft);
        padding: 0 18px 10px; cursor: pointer; user-select: none;
      }
      .receipt-thermal-toggle input { margin: 0; }
      ...
      @media print {
        ...
        /* Format ticket de caisse thermique 80mm : papier étroit, police
           compacte, sans marges de page superflues. */
        .receipt-thermal { width: 72mm; padding: 4mm 3mm; font-size: 10.5px; }
        .receipt-thermal .receipt-brand { font-size: 13px; }
        .receipt-thermal .receipt-table th, .receipt-thermal .receipt-table td { padding: 2px 1px; font-size: 9.5px; }
        .receipt-thermal .receipt-total-row { font-size: 12px; }
      }
